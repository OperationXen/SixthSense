...
function SixthSense_StateModel:add_targeting_hostile(friendly_unit_id, guid)
	local model_changed = false
	...
	if self.model[friendly_unit_id].hostiles[guid] == nil then			-- check that we're not about to attempt to duplicate information	
		self.model[friendly_unit_id].hostiles[guid] = true				-- add the hostile guid to the list of targeting units
...
function SixthSense_StateModel:process_target_info(targeting_unit_id, target_guid)
	...
	for friendly_unit_id, data in pairs(self.model) do						
		if data.guid == target_guid	then										-- if the target_guid matches someone in our model
			...
			self:add_targeting_hostile(friendly_unit_id, hostile_unit_guid)		-- update model, friendly player now targetted by hostile
				
			print(friendly_unit_id .. " is now targeted by " .. hostile_unit_guid)
		end
	end
end
-- #####################################################################################################